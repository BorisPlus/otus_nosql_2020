...
__ЗАМЕЧАНИЕ__ на одной из страниц указано `PHONE> do ##class(Shop.Company).Populate(10)`. Представляется более понятным вариант `SHOP> ...`, так как о `PHONE...` мы ничего в методичке "не знаем".
...
Их состояние - сначала в списке Order, а потом его OrderItem
