...
__ВНИМАНИЕ__: в отчете заменил значания идентификаторов в Яндекс.Облаке на их "<ШАБЛОНЫ>"
...
__ВНИМАНИЕ__: в доке вроде нет, после приведенной выше команды нужно `source "/home/<USER>/.bashrc"`.
...
Сделал отдельный скрипт импорта, который учитывает ["новый" подход](https://habr.com/ru/company/postgrespro/blog/353472/) в разбиении на партиции. Демонстрация ниже (даты партиций нам известны, с целью ускорения импортирую первые несколько столбцов, а не все), но она полностью с импортом включена в [скрипт](./pg_import.py).
